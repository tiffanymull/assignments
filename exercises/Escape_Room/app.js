...
//Could make a series of functions up here for what you want to happen and call on them as you go through the story.
...
// }
//write out story in functions and then below them, call them in the order I want them to happen.
//1.
...
//2.
    ...
    if (handHole === 'Yes'){
        ...
        break; //<-- not necessarily needed bc isAlive = false takes care of it.
    } else if (handHole === 'No'){
...
//3.
while(!hasKey){  //<-- same as saying while (hasKey === false)
    ...
    if (lookOrNot === 'Yes'){
        console.log('Good idea. ')    //<-- could've put var lookWhere into a function called lookWhere or whatever
...
//5.
                ...
                while(indecisive === true) {    
                    var openDoor = readline.question('Now that you have the key, are you going to try the door? Type Yes or No.')
                        ...
                        console.log('You try the key. It worked! The door opened! Unfortunately, there is a murderous clown on the other side. Have fun!')
                            indecisive = false;  //<-- could've used break;
                        } else if (openDoor === 'No'){
                        console.log('Though you have the key, you are too scared to try the door. You wet yourself. And died.') //<--make it loop back or kill them
                            indecisive = false;
    ...
    console.log('Rather than look for a key, you stand there staring. Nothing happens. You\'re getting pretty thirsty. ')  //<-- I need to get this to loop back up into the question
    } else {
